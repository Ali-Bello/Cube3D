...
/*   Updated: 2025/01/27 20:22:25 by aderraj          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_collectibles(t_bonus_game *game, int id, char *path)
{
	...
	game->world[id].n_coins = 1 + (rand() % (int)fmax(1, (int)((game->data.map_height * 2) * 0.5)));
...
{
	if (load_image(&game->data, &game->world[0].textures[0],
		parse->no_text)
...
int	init_bonus_game(t_bonus_game *game, t_parse *parse)
{
	...
	if (load_bonus_textures(game, parse)
